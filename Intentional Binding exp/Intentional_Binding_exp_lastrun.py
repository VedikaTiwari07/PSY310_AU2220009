...
expName = 'Task6_IntentionalBinding'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\vedik\\OneDrive\\Desktop\\AU\\PSY310 Lab\\Intentional Binding exp\\Intentional_Binding_exp_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler(nReps=25.0, method='random', 
        ...
        trialList=data.importConditions('Intentional Binding_conditions.xlsx'),
    ...
    for thisTrial in trials:
        ...
        trialComponents = [fixation, polygon, key]
        ...
        while continueRoutine:
            ...
            if key.status == NOT_STARTED and tThisFlip >= 7.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key.status == STARTED and not waitOnFlip:
                theseKeys = key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
        ...
        if key.keys != None:  # we had a response
            ...
            trials.addData('key.duration', key.duration)
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Beep_Sound.started', globalClock.getTime(format='float'))
        ...
        for thisComponent in Beep_SoundComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Beep_Sound.stopped', globalClock.getTime(format='float'))
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Responses.started', globalClock.getTime(format='float'))
        textbox.reset()
        # reset button to account for continued clicks & clear times on/off
        button.reset()
        # keep track of which components have finished
        ResponsesComponents = [text, textbox, button]
        ...
        for thisComponent in ResponsesComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Responses.stopped', globalClock.getTime(format='float'))
