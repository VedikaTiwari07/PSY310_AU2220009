...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\vedik\\OneDrive\\Desktop\\AU\\PSY310 Lab\\Solo Project\\SOLO PROJECT_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='Hello! \n\nYou will be presented with a series of images after this screen. Press "M" if the objects in the image are animate and "N" if they are inanimate. Try to respond as accurately as possible. \n\nThank you so much for your participation!',
        font='Arial',
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('solo proj_conditions.csv'),
    ...
    for thisTrial in trials:
        ...
        key_resp_2.rt = []
        _key_resp_2_allKeys = []
        stimulusImage.setImage(stimImage)
        # keep track of which components have finished
        trial_routineComponents = [fixation, key_resp_2, stimulusImage]
        ...
        while continueRoutine:
            ...
            if fixation.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixation.tStartRefresh + 0.03-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp_2.status == NOT_STARTED and tThisFlip >= 1.03-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key_resp_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_2.getKeys(keyList=['m','n'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if stimulusImage.status == NOT_STARTED and tThisFlip >= 0.03-frameTolerance:
                # keep track of start time/frame for later
            ...
            if stimulusImage.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > stimulusImage.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
